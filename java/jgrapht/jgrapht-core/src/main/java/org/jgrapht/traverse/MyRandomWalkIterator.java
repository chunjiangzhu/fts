 ...
 * @param <E> edge type
 */
public class MyRandomWalkIterator<V, E>
    extends
    AbstractGraphIterator<V, E>
{
    ...
    // Chunjiang added 20190109. probability of random walk. See Section 4 in http://proceedings.mlr.press/v51/sadhanala16.pdf
    private double pi;
    private Map<V, Integer> count;
     ...
     */
    public MyRandomWalkIterator(Graph<V, E> graph, Map<V, Integer> count)
    {
        this(graph, null, count);
    }
     ...
     *         <code>startVertex</code>
     */
    public MyRandomWalkIterator(Graph<V, E> graph, V startVertex, Map<V, Integer> count)
    {
        this(graph, startVertex, count, true);
    }
     ...
     *         <code>startVertex</code>
     */
    public MyRandomWalkIterator(Graph<V, E> graph, V startVertex, Map<V, Integer> count, boolean isWeighted)
    {
        this(graph, startVertex, count, isWeighted, Long.MAX_VALUE);
    }
     ...
     *         <code>startVertex</code>
     */
    public MyRandomWalkIterator(Graph<V, E> graph, V startVertex, Map<V, Integer> count, boolean isWeighted, long maxSteps)
    {
        this(graph, startVertex, count, isWeighted, maxSteps, new Random());
    }
     ...
     *         <code>startVertex</code>
     */
    public MyRandomWalkIterator(
        Graph<V, E> graph, V startVertex, Map<V, Integer> count, boolean isWeighted, long maxSteps, Random rng)
    {
        super(graph);
        ...
        // Chunjiang revised to true random, 20190109
        // select a random start vertex in case not provided.
        if (startVertex == null) {
            if (graph.vertexSet().size() > 0) {
            	Iterator<V> itr = graph.vertexSet().iterator();
            	int i = random.nextInt(graph.vertexSet().size());
            	for(int j=0; j<i; j++)
        ...
        // pi probability of continuing or terminating a random walk
        this.pi = random.nextDouble() * 0.5;
        this.count = count;
    }
    ...
    protected void encounterVertex(V vertex, E edge)
    {
        ...
        count.put(vertex, count.get(vertex)+1);
    }
    ...
    // Chunjiang revised 20190109
    // With prob. 1-pi, goes to a random neighbor, terminates otherwise
    /**
